...
use evm_signer_kms::{
    ...
    transaction::legacy_transaction::LegacyTransaction,
};
use std::env;
...
async fn main() -> Result<()> {
    ...
    let unigned_tx = serde_json::from_str::<LegacyTransaction>(LEGACY_TX_JSON).or_else(|err| {
    ...
    println!("Encoded signed legacy transaction: {signed_tx_encoding}");
